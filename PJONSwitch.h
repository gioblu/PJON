...
possible to use a PJONSwitch to handle leaf buses in a tree structure.
...
protected:
  ...
  virtual void dynamic_receiver_function(
    ...
    do {
      uint8_t receiver_bus = find_bus_with_id(
...
public:
  ...
  static void receiver_function(
    ...
    ((PJONSwitch*) packet_info.custom_pointer)->dynamic_receiver_function(
      payload,
      length,
      packet_info
    );
