...
- **Increased security**: Ethernet and WiFi are exposing devices to ransomware, illegal cyber warfare and private data leak risks. PJON has been engineered to enhance security not necessarily implementing the standard network protocol stack together with its vulnerabilities where it is not absolutely necessary, offering a set of alternatives for many use cases.
