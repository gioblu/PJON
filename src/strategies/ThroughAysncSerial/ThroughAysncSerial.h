...
public:
  ...
  /* Check if the channel is free for transmission: This might be changable... https://www.arduino.cc/en/Serial/AvailableForWrite*/
  ...
  uint16_t receive_string(uint8_t *string, uint16_t max_length)
  {
    ...
    case TAS_RECEIVING: {
      ...
      while (PJON_SERIAL_AVAILABLE(serial)) {
        ...
        if (value == TAS_FAIL) { state = TAS_WAITING; return TAS_FAIL; }   //  byte consumed
        // Unescaped START byte stuffing violation
        if (value == TAS_START) { state = TAS_WAITING; return TAS_FAIL; }  //  byte consumed
